...
/// <summary>
/// Command to list queues in a solution.
/// </summary>
public static class QueuesCommand
{
    public static Command Create()
    {
        var command = new Command("queues", "List all queues in a solution");
    ...
    private static async Task<int> ExecuteAsync(InvocationContext context, string solutionName)
    {
        ...
        var logger = loggerFactory.CreateLogger("queues");
        ...
        try
        {
            ...
            // Execute query
            logger.LogInformation("Retrieving queues for solution: {Solution}", solutionName);
            var queues = await reader.GetQueuesAsync(solutionName, context.GetCancellationToken());
